...
"""
    ...
    def test_parse_prepare_as_list_of_tuples(self):
        cluster = "my-cluster"
        region = "europe-west3"
        job_properties = {"nameNode": "hdfs://localhost:8020"}
        config = {"dataproc_cluster": cluster, "gcp_region": region}
...
<pig>
    ...
    <prepare>
        ...
        self.assertTrue(extension.has_prepare())
        tuples = extension.parse_prepare_as_list_of_tuples()
        self.assertEqual(
            [
    ...
    def test_parse_prepare_as_list_of_tuples_should_raise_exception_when_invalid_node_is_given(self):
        cluster = "my-cluster"
        region = "europe-west3"
        job_properties = {"nameNode": "hdfs://localhost:8020"}
        config = {"dataproc_cluster": cluster, "gcp_region": region}
    ...
    <pig>
        <name-node>hdfs://</name-node>
        <prepare>
            <mkdir path='test'></mkdir>
            <delet path="${nameNode}/examples/output-data/demo/pig-node" />
        </prepare>
    </pig>
            """
        pig_node = ET.fromstring(pig_node_str)
        ...
        with self.assertRaises(Exception):
            extension.parse_prepare_as_list_of_tuples()
