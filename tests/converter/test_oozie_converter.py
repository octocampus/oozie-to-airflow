...
from o2a.converter.task_group import TaskGroup, ActionTaskGroup, ControlTaskGroup
...
class TestOozieConverter(TestCase):
    ...
    def test_convert_relations(self):
        ...
        end = TaskGroup(dag_id="test_dag", name="end1", tasks=[self._create_task("end1")])
        fail = TaskGroup(dag_id="test_dag", name="fail1", tasks=[self._create_task("fail1")])
        ...
        self.assertEqual(
            workflow.task_group_relations,
            {
                ...
                Relation(from_task_id="task2_branch", to_task_id="end1", is_error=False),
                Relation(from_task_id="task2_branch", to_task_id="fail1", is_error=True),
                Relation(from_task_id="task3_branch", to_task_id="end1", is_error=False),
                Relation(from_task_id="task3_branch", to_task_id="fail1", is_error=True),
            },
        )

    def test_add_error_handlers(self):
        task_groups = {
            "TASK_A": create_autospec(spec=ActionTaskGroup, instance=True),
            "TASK_B": create_autospec(spec=ControlTaskGroup, instance=True),
        }
        converter = self._create_converter()
        ...
        task_groups["TASK_A"].add_state_handler.assert_called_once_with()
        task_groups["TASK_B"].add_state_handler.assert_not_called()
        ...
        self.assertEqual(
            {
                ...
                Relation(from_task_id="hdfs-node_branch", to_task_id="end", is_error=False),
                Relation(from_task_id="hdfs-node_branch", to_task_id="fail", is_error=True),
                Relation(from_task_id="join-node", to_task_id="decision-node", is_error=False),
                Relation(from_task_id="pig-node_branch", to_task_id="join-node", is_error=False),
                Relation(from_task_id="pig-node_branch", to_task_id="fail", is_error=True),
                Relation(from_task_id="shell-node_branch", to_task_id="join-node", is_error=False),
                Relation(from_task_id="shell-node_branch", to_task_id="fail", is_error=True),
                Relation(from_task_id="subworkflow-node_branch", to_task_id="join-node", is_error=False),
                Relation(from_task_id="subworkflow-node_branch", to_task_id="fail", is_error=True),
            },
            workflow.task_group_relations,
        )
        ...
        self.assertEqual(
            {
                ...
                "from o2a.o2a_libs.operators.oozie_operators import ShellOozieOperator",
                "from o2a.o2a_libs.property_utils import PropertySet",
                "from o2a.o2a_libs.utils import get_upstream_state",
                "import datetime",
                "import pendulum",
                "import shlex",
            },
