...
class AddNodeNotificationTransformer(BaseWorkflowTransformer):
    ...
    def _add_status(self, task_group, workflow, upstream=None):
        status_notification = self._create_status_notification_task_group(
            ...
            f"{task_group.name}{NODE_STATUS_SUFFIX}",
            task_group.name,
            "S:RUNNING",
            workflow,
        )
    ...
    def _add_transition_and_status(self, downstream_name, task_group, workflow):
        ...
        status_notification = self._create_status_notification_task_group(
            self.notification_url,
            f"{downstream_name}{NODE_STATUS_SUFFIX}",
            downstream_name,
            "S:RUNNING",
            workflow,
        )
    ...
    @staticmethod
    def _create_status_notification_task_group(url_template, task_name, node_name, status, workflow):
        url = (
    ...
    @staticmethod
    def _create_transition_notification_task_group(url_template, task_name, node_name, status, workflow):
        url = (
            url_template.replace("$jobId", "{{ dag.dag_id }}")
            .replace("$nodeName", node_name)
            .replace("$status", status)
