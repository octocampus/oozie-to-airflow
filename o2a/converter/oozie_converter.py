...
class OozieConverter:
    ...
    def convert(self, as_subworkflow=False):
        ...
        for key, val in self.workflow.nodes.items():
            print(key, " : ", end=" ")
            print(val)
        task_tree = self.get_tasks_tree()
        self.workflow.nodes = task_tree
        print("-----------")
        print()
        for key, val in self.workflow.nodes.items():
            print(key, " : ", end=" ")
    ...
    def convert_nodes(self):
        ...
        for name, oozie_node in self.workflow.nodes.copy().items():
            ...
            print(oozie_node.mapper.to_tasks_and_relations())
            self.workflow.task_groups[name] = self._get_task_group_type(oozie_node)(
                dag_id=self.workflow.dag_name,
                name=name,
                tasks=tasks,
                relations=relations,
    ...
    def convert_relations(self) -> None:
        ...
        for task_group in self.workflow.task_groups.values():
            for downstream in task_group.downstream_names:
                relation = Relation(
                    from_task_id=task_group.tasks[-1].task_id,
    ...
    def add_state_handlers(self) -> None:
        ...
        for node in self.workflow.task_groups.values():
            if isinstance(node, ActionTaskGroup):
                node.add_state_handler()
    ...
    def get_tasks_tree(self):
        ...
        i = 0
        root_copy = copy.deepcopy(root_node)
        root_copy.mapper.name = f"{root_copy.name}_{i}"
        root_copy.downstream_names = [f"{downstream}_{j + i + 1}" for j, downstream in
                                      enumerate(root_copy.downstream_names)]
        i += len(root_copy.downstream_names)
        if root_copy.error_downstream_name:
            root_copy.error_downstream_name = f"{root_copy.error_downstream_name}_{i + 1}"
            i += 1
        queue = [root_copy]
        while queue:
            current = queue.pop(0)
            task_tree[current.name] = current
            for dw in current.downstream_names:
                curr_dw = self.workflow.nodes[dw.rsplit("_", 1)[0]]
                curr_dw_copy = copy.deepcopy(curr_dw)
                curr_dw_copy.mapper.name = dw
                curr_dw_copy.downstream_names = [f"{downstream}_{j + i + 1}" for j, downstream in
                                                 enumerate(curr_dw_copy.downstream_names)]
                i += len(curr_dw_copy.downstream_names)
                if curr_dw_copy.error_downstream_name:
                    curr_dw_copy.error_downstream_name = f"{curr_dw_copy.error_downstream_name}_{i + 1}"
                    i += 1
                queue.append(curr_dw_copy)
            if current.error_downstream_name:
                error = self.workflow.nodes[current.error_downstream_name.rsplit("_", 1)[0]]
                error_copy = copy.deepcopy(error)
                error_copy.mapper.name = current.error_downstream_name
                error_copy.downstream_names = [f"{downstream}_{j + i + 1}" for j, downstream in
                                               enumerate(error_copy.downstream_names)]
                i += len(error_copy.downstream_names)
                if error_copy.error_downstream_name:
                    error_copy.error_downstream_name = f"{error_copy.error_downstream_name}_{i + 1}"
                    i += 1
                queue.append(error_copy)
        return task_tree


    def _get_root_node(self):
        for name, oozie_node in self.workflow.nodes.copy().items():
            if name.startswith("start_node"):
                return self.workflow.nodes[oozie_node.downstream_names[0]]
